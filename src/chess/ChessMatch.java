...
public class ChessMatch {
	...
	private void undoMove(Position source, Position target, Piece capturePiece) {
		...
		// # especial movimento castling kingside rook
		...
		// # especial movimento castling queenside rook
		if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
	...
	private ChessPiece king(Color color) {
		List<Piece> list = pieceOntheBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color)
				.collect(Collectors.toList()); // metodo filtra uma lista com mesma cor
		for (Piece p : list) {
	...
	private boolean testCheck(Color color) {
		...
		List<Piece> opponentPieces = pieceOntheBoard.stream()
				.filter(x -> ((ChessPiece) x).getColor() == opponent(color)).collect(Collectors.toList()); // Lista
																											// peças do
																											// oponente
																											// recebe a
																											// cor do
																											// OPPNENT
		for (Piece p : opponentPieces) { // Verifico que as peças oponenter estão com o destino na posição da peça do
											// rei
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> list = pieceOntheBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color)
				.collect(Collectors.toList()); // Pega todas peças tabu igual a cor
		for (Piece p : list) {
			boolean[][] mat = p.possibleMoves();
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if (mat[i][j]) {// esse movimento é possivel true
						Position source = ((ChessPiece) p).getChessPosition().toPosition(); // Possição de origem
																							// donwcastin
						Position target = new Position(i, j);
						Piece capturedPiece = makeMove(source, target); // fiz movimento da peça pra testa da origem e
																		// do destino, com testCheck
	...
	private void initialSetup() {
		...
		placeNewPiece('e', 1, new King(board, Color.WHITE, this)); // This no parametro é pra referencia a propria
																	// ChessMatch que foi implementa no construtor da
																	// Classe King
